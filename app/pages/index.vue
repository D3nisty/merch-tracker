...
<template>
  <div>
    ...
    <div v-if="store.events.length === 0 && !store.loading" class="text-center py-20">
      ...
      <h2 class="text-xl font-semibold text-gray-400 mb-2">No events yet</h2>
      <p class="text-gray-500 mb-6">Create your first convention or travel event to start tracking merch</p>
      <UButton to="/events/create" icon="i-heroicons-plus" color="purple">Create Event</UButton>
    </div>
    ...
    <section v-if="conventionEvents.length > 0" class="mb-10">
      <div class="flex items-center gap-2 mb-4">
        ...
        <h2 class="text-xl font-bold text-white">Conventions</h2>
    ...
    <section v-if="travelEvents.length > 0">
      <div class="flex items-center gap-2 mb-4">
        ...
        <h2 class="text-xl font-bold text-white">Travel</h2>
        <UBadge :label="String(travelEvents.length)" color="blue" variant="soft" />
      </div>
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <EventCard
          v-for="event in travelEvents"
          :key="event.id"
          :event="event"
          @delete="confirmDelete(event.id)"
        />
    ...
    <UModal v-model="showDeleteModal" :ui="{ width: 'sm:max-w-sm' }">
      <UCard>
        <template #header>
          <h3 class="font-semibold text-white">Delete Event?</h3>
        </template>
        <p class="text-gray-400 text-sm">This will permanently delete all locations, booths and products.</p>
        <template #footer>
