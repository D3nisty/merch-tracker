...
| `itinerary_items` | Travel-planner entries per city (`location_id` + `event_id`, both CASCADE). One unified table for the whole planner: `kind` (`activity`/`ticket`/`food`/`transport`/`shopping`/`note`), `title`, optional `date`+`time` (drives the day-by-day itinerary; null = "anytime"), `done` (checkbox / "bought"), `price`+`currency` (feeds the per-city budget for `shopping` items), `url` (openable link) and `attachment_path`+`attachment_name` (an uploaded ticket/reservation file or **QR/screenshot image**, openable from the plan). Also: `locations` gained `transport` + `accommodation` free-text logistics columns. Endpoints: `POST /api/itinerary`, `PUT/DELETE /api/itinerary/[id]`, `POST /api/upload/itinerary-attachment` (multipart; replaces prior attachment) — all gated by `requireEventEdit` via `eventIdForLocation`/the item's `eventId`. `GET /api/events/[id]` returns `itinerary: ItineraryItem[]` + `transport`/`accommodation` per location. Cascade-delete walkers (event + location DELETE) collect `attachment_path` to unlink files. Surfaced via [CityPlannerModal.vue](../app/components/CityPlannerModal.vue) — opened by a calendar button on each travel `LocationCard` — with logistics, day-grouped itinerary (tickets show an openable QR/attachment + link), a shopping wishlist, and a per-city budget (rolled up from the city's booth products via the existing `getBooth*ByCurrency` helpers **plus** wishlist items). **Multiple attachments** per item live in `itinerary_attachments` (id, item_id CASCADE, path, name, sortOrder) — `POST /api/upload/itinerary-attachment` appends N files, `DELETE /api/itinerary/attachments/[id]` removes one; the GET nests `attachments[]` per item. **Transport legs** (`kind='transport'`) use `from_loc`/`to_loc` + `time` (departure) / `end_time` (arrival). **Trip map**: `locations` gained `latitude`/`longitude`, geocoded on demand via `POST /api/locations/[id]/geocode` ([server/utils/geocode.ts](../server/utils/geocode.ts), OpenStreetMap Nominatim, ≤1 req/s so the map throttles); [TripMapModal.vue](../app/components/TripMapModal.vue) (Leaflet, dynamically imported, CartoDB dark tiles) draws numbered city pins + a route polyline. **Timetable view**: the event page has an Overview↔Timetable toggle (travel only); [TripTimetable.vue](../app/components/TripTimetable.vue) flattens every city's itinerary into a chronological day-by-day schedule (transport, tickets, activities, plus a per-city "stay begins" marker). The timetable is fully **editable + addable in place**: an "Add entry" button (header + per-day ＋), and clicking any row opens the shared editor. **[ItineraryItemModal.vue](../app/components/ItineraryItemModal.vue)** is the single add/edit surface for one itinerary item — city picker (locked in edit / when opened from a city), kind, title, transport from→to, date/time/arrival, price+currency, url, notes, and multi-attachment management (pending files buffer on create, live upload/remove on edit). It's reused by both the timetable (click-to-edit + add) and [CityPlannerModal.vue](../app/components/CityPlannerModal.vue) (pencil button on each itinerary/wishlist row). **Excel/CSV import** ([ImportTravelModal.vue](../app/components/ImportTravelModal.vue) + [useItineraryImport.ts](../app/composables/useItineraryImport.ts)): parses an `.xlsx`/`.csv` day→place sheet client-side (fflate unzip, no upload) into city blocks with date ranges + per-day notes. Two modes: **create** (from the create page — has a travel/convention **type toggle** + trip-name field) and **append** (Import button on a travel event header — `:target-event`). A **preview** shows exactly what will be added: every stop is expandable to its day-items, each flagged ADD or SKIP. **De-dupe** (append only): a stop is matched to an existing location by `norm(name)` + identical `dateFrom`/`dateTo`; matched stops merge (adding only day-items whose `date|norm(title)` key isn't already present), fully-duplicate stops default to unchecked. Commit fans out `POST /api/locations` (skipped for matched stops) + `POST /api/itinerary` per new day-item, then `store.fetchEvent` to refresh (append) or routes to the new event (create). Convention imports create `hall` locations and skip itinerary items. |
