...
| **Settlements** (computed, no table) | "Who owes who" math. `store.perReceiptDebts(receipt, location)` returns one entry per non-payer person who has assigned items, with a per-currency map of what they owe the receipt's payer. `store.eventSettlements()` rolls those up across every receipt in the event AND nets reverse flows (A→B and B→A on the same pair cancel out). Surfaces: bottom of the receipt modal AND a settlements UCard on the event page (between the totals row and the locations list), shown only when `settlements.length > 0`. Per-currency only — pairs are NOT cross-converted; the [`PriceConverted`](../app/components/PriceConverted.vue) helper adds an `≈ €X` caption when more than one currency is involved. |
