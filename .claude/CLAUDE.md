...
| `products` | Items to buy, with price, size, category, optional region overlay, `ownerId` (creator) for visibility filtering. `is_planned`/`is_purchased` are now MAINTAINED AS ANY-PERSON AGGREGATES (server-side) but the API substitutes them with the *requesting user's* per-person mark when returning the event tree — so existing single-perspective UIs (checkboxes, strike-through) keep working without per-call refactors. **`split_among_marked`** (boolean) flips settlement math: when true AND the product has ≥2 purchased markers on a receipt, the line total `price × sum(per-person quantities)` is divided equally across markers (group-shared item); when false (default), each marker owes `price × their own quantity`. Toggled per-row via the 🔀 button in `LocationReceiptModal.vue`. |
